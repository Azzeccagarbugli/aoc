...
void main() {
  ...
  final limit = pairs.length < 1000 ? pairs.length : 1000;
  for (var k = 0; k < limit; k++) {
    unite(pairs[k].$2, pairs[k].$3);
  }

  final compSizes = <int>[];
  for (var i = 0; i < n; i++) {
    if (find(i) == i) compSizes.add(size[i]);
  }

  compSizes.sort((b, a) => a.compareTo(b));
  while (compSizes.length < 3) {
    compSizes.add(1);
  }
  final product = compSizes.take(3).reduce((a, b) => a * b);
