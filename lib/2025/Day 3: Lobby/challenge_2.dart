...
void main() {
  ...
  for (var line in input) {
    ...
    final maxJoltage = _findMaxJoltage12(intCurrentLine);
...
int _findMaxJoltage12(List<int> digits) {
  const targetCount = 12;

  if (digits.length <= targetCount) {
  ...
  for (int pos = 0; pos < targetCount; pos++) {
    int endIndex = digits.length - (targetCount - pos);
  ...
  return int.parse(selected.join(''));
}
