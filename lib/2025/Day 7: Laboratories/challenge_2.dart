...
void main() {
  final grid = File('lib/2025/Day 7: Laboratories/input.txt').readAsLinesSync();
  final width = grid[0].length;

  var pathCounts = {grid[0].indexOf('S'): 1};

  for (var row = 1; row < grid.length; row++) {
    final nextCounts = <int, int>{};

    for (final MapEntry(:key, :value) in pathCounts.entries) {
      if (key < 0 || key >= width) continue;

      if (grid[row][key] == '^') {
        nextCounts[key - 1] = (nextCounts[key - 1] ?? 0) + value;
        nextCounts[key + 1] = (nextCounts[key + 1] ?? 0) + value;
      } else {
        nextCounts[key] = (nextCounts[key] ?? 0) + value;
      }
    }
  ...
  print(pathCounts.values.reduce((a, b) => a + b));
}
